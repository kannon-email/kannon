...
// The deprecated prefix still reaches the keys it always reached, and now through
// the struct the root configuration is read into.
func TestRead_HonoursTheDeprecatedEnvPrefix(t *testing.T) {
	viper.Reset()
	t.Cleanup(viper.Reset)
	...
	if cfg.DatabaseURL != "postgres://legacy@db/kannon" {
		t.Errorf("DatabaseURL = %q, want the value of K_DATABASE_URL", cfg.DatabaseURL)
	}
