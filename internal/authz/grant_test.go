...
// Every refusal, with the message asserted where the message is the point.
//
// An unhelpful refusal here is a real cost: this is the error an operator sees
// when issuing a credential, and the near misses — the bare domains collection, a
// typed Role on the wrong kind of Anchor — are exactly the ones whose *meaning*
// would otherwise differ silently from what was intended.
func TestNewGrantRefuses(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			msg:    `anchor "domains/example.com/batches" is not grantable; a Grant is issued on the root ("*") or on a Domain ("domains/<fqdn>" or "domains/*")`,
		},
		{
			name:   "one Template",
			role:   authz.RoleAdmin,
			anchor: authz.AnchorOf(authz.Template(example, "welcome")),
			msg:    `anchor "domains/example.com/templates/welcome" is not grantable; a Grant is issued on the root ("*") or on a Domain ("domains/<fqdn>" or "domains/*")`,
		},
		{
			name:   "a Templates collection",
			role:   authz.RoleAdmin,
			anchor: authz.AnchorOf(authz.Templates(example)),
			msg:    `anchor "domains/example.com/templates" is not grantable; a Grant is issued on the root ("*") or on a Domain ("domains/<fqdn>" or "domains/*")`,
		},
		{
			name:   "an API Key",
			role:   authz.RoleAdmin,
			anchor: authz.AnchorOf(authz.APIKey(example, "key-1")),
			msg:    `anchor "domains/example.com/apikeys/key-1" is not grantable; a Grant is issued on the root ("*") or on a Domain ("domains/<fqdn>" or "domains/*")`,
		},
		{
			name:   "the counters",
			role:   authz.RoleAdmin,
			anchor: authz.AnchorOf(authz.AggregatedStats(example)),
			msg:    `anchor "domains/example.com/stats/aggregated" is not grantable; a Grant is issued on the root ("*") or on a Domain ("domains/<fqdn>" or "domains/*")`,
		},
		{
			...
			role:   authz.RoleAdmin,
			anchor: authz.Anchor{},
			msg:    `anchor "" is not grantable; a Grant is issued on the root ("*") or on a Domain ("domains/<fqdn>" or "domains/*")`,
		},
		{
		...
		{
			// Shaped like a Domain Anchor and refused anyway: a zero FQDN
			// reaching a constructor is a programming error, and it must not
			// yield a Grant that reaches whatever "domains/" alone would.
			name:   "a Domain with a zero FQDN",
			role:   authz.RoleAdmin,
			anchor: authz.DomainAnchor(values.DomainName{}),
			msg:    `anchor "domains/" is not grantable; a Grant is issued on the root ("*") or on a Domain ("domains/<fqdn>" or "domains/*")`,
		},
		{
			// The soundness case ADR 0008 owes the model: a typed Role granted
			// off its declared kind. sender's rule names batches, which composes
			// only beneath a Domain — at the root it would compose the bare
			// "batches", a path that means nothing here.
