...
func (q *Queries) CountTemplates(ctx context.Context, domain string) (int64, error) {
	row := q.db.QueryRow(ctx, countTemplates, domain)
	var count int64
	err := row.Scan(&count)
	return count, err
}
...
func (q *Queries) DeleteTemplate(ctx context.Context, templateID string) (Template, error) {
	row := q.db.QueryRow(ctx, deleteTemplate, templateID)
	var i Template
	err := row.Scan(
		&i.ID,
		&i.TemplateID,
...
func (q *Queries) GetTemplates(ctx context.Context, arg GetTemplatesParams) ([]Template, error) {
	rows, err := q.db.Query(ctx, getTemplates, arg.Domain, arg.Skip, arg.Take)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (q *Queries) UpdateTemplate(ctx context.Context, arg UpdateTemplateParams) (Template, error) {
	row := q.db.QueryRow(ctx, updateTemplate, arg.TemplateID, arg.Html, arg.Title)
	var i Template
	err := row.Scan(
		&i.ID,
		&i.TemplateID,
