...
- Database access layer (generated by sqlc). Contains models and queries for all main tables: domains, messages, sending_pool_emails, templates, stats, stats_keys. Hosts the sqlc-backed `Repository` implementations for the `apikeys`, `batch`, `delivery`, `domains`, and `templates` domain packages, translating between sqlc rows and domain entities at the boundary.
